    ...
    for z, P_wzi in enumerate(P_wz):
        ...
        if verbose:
            print('Topic {} | Coherence={:0.2f} | Top words= {}'.format(
                z, C_z, ' '.join(W_z)))
    print('Average topic coherence for the top words is {}'.format(
        sum(res['coherence'])/len(res['coherence'])))
    return res
...
class Lexicon:
    ...
    def __init__(self, files):
        ...
        for file in files:
            self._sent_category.append(
                set([self.ps.stem(line.strip().lower()) for line in open(file)]))
